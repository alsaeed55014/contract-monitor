...
def render_whatsapp_page():
    ...
    lbl = {
        ...
        'delete_log': "🗑️ حذف السجل" if is_ar else "🗑️ Clear Log",
        ...
        'batch_delay': "مدة الاستراحة (ثواني)" if is_ar else "Pause duration (seconds)",
        'pausing': "⏳ استراحة مؤقتة لمدة {} ثانية..." if is_ar else "⏳ Pausing for {} seconds...",
        'next_msg_in': "⏳ الرسالة القادمة خلال {} ثانية..." if is_ar else "⏳ Next message in {}s...",
...
Abu Fahd"""
        ...
        with col_s1:
            delay = st.slider(lbl['delay'], 5, 120, 15, disabled=st.session_state.wa_running)
        with col_s2:
        ...
        with col_s3:
            batch_delay = st.number_input(lbl['batch_delay'], min_value=10, max_value=3600, value=60, disabled=st.session_state.wa_running)
                ...
                if st.session_state.wa_idx == len(final_targets):
                    ...
                    if batch_size > 0 and st.session_state.wa_idx % batch_size == 0:
                        ...
                        for i in range(batch_delay, 0, -1):
                            ...
                            wait_ph.warning(lbl['pausing'].format(i))
                            time.sleep(1)
                        wait_ph.empty()
                    else:
                        wait_ph = st.empty()
                        for i in range(delay, 0, -1):
                            if not st.session_state.wa_running: break
                            wait_ph.info(lbl['next_msg_in'].format(i))
                            time.sleep(1)
                        wait_ph.empty()

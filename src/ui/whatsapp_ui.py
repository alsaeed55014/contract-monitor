...
def render_whatsapp_page():
    ...
    with c1:
        if status == "Connected": st.success("✅ متصل يا باشا!")
    ...
    # 2. QR CODE - PIXEL-PERFECT DISPLAY
    if status == "Awaiting Login":
        qr_b64 = st.session_state.wa_service.get_qr_hd()
        if qr_b64:
            # Check if it's a data URL (fallback) or pure base64 (PIL method)
            if qr_b64.startswith("data:"):
                src = qr_b64
            else:
                src = f"data:image/png;base64,{qr_b64}"
            
            # Display using raw HTML with image-rendering: pixelated
            # This tells the browser to NOT smooth/blur the pixels
            st.markdown(f"""
            <div style="
                background: #FFFFFF;
                padding: 25px;
            ...
            ">
                <img src="{src}" 
                     ...
                     " />
                <div style="color:#333; font-size:14px; margin-top:10px; font-family:Cairo;">
                    📱 امسح الباركود من واتساب الموبيل يا باشا
                </div>
            </div>

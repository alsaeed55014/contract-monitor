...
def render_whatsapp_page():
    ...
    with col_input:
        st.markdown('<div class="wa-glass-card">', unsafe_allow_html=True)
        st.subheader("Step 1: Numbers")
        file_up = st.file_uploader(t("wa_upload_opt", lang), type=['xlsx', 'csv', 'txt'])
        txt_in = st.text_area("Paste Numbers Here", height=150, placeholder="054...")
        ...
        v_sa, v_ph, _ = validate_numbers(final_nums)
        all_v = v_sa + v_ph
        if all_v: st.success(f"Verified Numbers: {len(all_v)}")
        st.markdown('</div>', unsafe_allow_html=True)

    with col_msg:
        st.markdown('<div class="wa-glass-card">', unsafe_allow_html=True)
        st.subheader("Step 2: Message")
        msg_txt = st.text_area("Write Your Message", height=150)
        delay = st.slider("Delay (Sec)", 2, 60, 5)
        st.markdown('</div>', unsafe_allow_html=True)

    # ACTION AREA - ALWAYS ENABLED FOR PASHA
    st.markdown('<div class="wa-glass-card" style="text-align:center;">', unsafe_allow_html=True)
    if st.session_state.wa_running:
        if st.button("🛑 STOP BROADCAST", type="primary", use_container_width=True):
            st.session_state.wa_running = False; st.rerun()
    else:
        # Button is ALWAYS ENABLED. If clicked, we check conditions INSIDE.
        if st.button(f"🚀 {t('wa_start_btn', lang)} 🔥", use_container_width=True):
            if status != "Connected":
                st.error("❌ Pasha! Please connect WhatsApp Web first.")
            elif not all_v:
                st.error("❌ Pasha! Please add at least one valid number.")
            else:
    ...
    if st.session_state.wa_running and all_v:
        ...
        if st.session_state.wa_idx < len(all_v):
            ...
            st.info(f"🚀 Sending for معاليك يا باشا: {target}...")
            
            try:
                final_body = msg_txt if msg_txt.strip() else "Hello from My App"
                ...
                st.session_state.wa_logs.append(f"[{datetime.now().strftime('%H:%M')}] ❌ Error: {str(e)}")
                st.session_state.wa_idx += 1

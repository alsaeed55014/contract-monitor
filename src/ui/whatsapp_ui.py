...
def get_whatsapp_ui_css(lang):
    ...
    <style>
        ...
        .wa-log {{
            ...
            overflow-y: auto;
            color: #aaa;
            direction: ltr; /* Logs always LTR for technical consistency */
        }}
    </style>
    """
...
def render_whatsapp_page():
    ...
    with col_status:
        ...
        if status == "Connected":
            ...
            if qr_b64:
                ...
                st.warning("QR Not Loaded. Loading...")
                time.sleep(2)
                st.rerun()
        elif status == "Loading...":
            ...
            if st.button(t("wa_launch_btn", lang), use_container_width=True):
                with st.spinner(t("extracting", lang)):
                    success, info = st.session_state.wa_service.start_driver(headless=False)
                    if not success:
    ...
    with col_main:
        ...
        if st.session_state.wa_logs:
            ...
            st.markdown('</div>', unsafe_allow_html=True)
            if st.button(t("wa_export_report", lang)):

...
def render_whatsapp_page():
    ...
    # Sidebar Reset Button (MUST BE IN SIDEBAR)
    st.sidebar.markdown("---")
    if st.sidebar.button("🔄 Reset Global WhatsApp", key="reset_wa_glob"):
        if st.session_state.wa_service.driver:
            st.session_state.wa_service.close()
        st.session_state.wa_service = WhatsAppService()
        st.sidebar.success("WhatsApp Reset Correctly!")
        st.rerun()
    ...
    with col_status:
        ...
        if status == "Connected":
            ...
            if qr_b64:
                st.markdown('<div style="text-align:center;" class="wa-qr-box">', unsafe_allow_html=True)
                st.image(f"data:image/png;base64,{qr_b64}", width=230)
                st.markdown('</div>', unsafe_allow_html=True)
                time.sleep(5)
                st.rerun()
            else:
                st.warning("QR is loading... wait 10s")
                time.sleep(3)
                st.rerun()
        elif status == "Loading...":
    ...
    with col_main:
        ...
        st.slider(t("wa_delay_lbl", lang), 2, 60, 5)
        st.markdown('</div>', unsafe_allow_html=True)

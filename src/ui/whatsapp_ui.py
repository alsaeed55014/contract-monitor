...
def get_whatsapp_ui_css(lang):
    ...
    <style>
        ...
        .status-box {{
            padding: 15px; border-radius: 15px; text-align: center; font-weight: bold;
            border: 1px solid rgba(255,255,255,0.1); margin-bottom: 20px;
        }}
        .wa-status-connected {{ color: #00FF41; font-size: 1.3rem; text-shadow: 0 0 10px rgba(0, 255, 65, 0.3); }}
    </style>
    """
...
def render_whatsapp_page():
    ...
    # Pasha's Service Initialization
    ...
    # 1. Connection Section (Pasha's Control)
    status = st.session_state.wa_service.get_status()
    st.markdown('<div class="wa-glass-card">', unsafe_allow_html=True)
    c1, c2 = st.columns([2, 1])
    with c1:
        if status == "Connected": st.markdown(f'<div class="wa-status-connected">✅ {t("wa_online", lang)}</div>', unsafe_allow_html=True)
    ...
    # 3. Main Form (ALWAYS VISIBLE FOR PASHA)
        ...
        if file_up:
            try:
                if file_up.name.endswith('.xlsx'):
                    ...
                    cols = [c for c in df.columns if any(k in str(c).lower() for k in ['phone', 'num', 'mob', 'جوال', 'هاتف'])]
                    if cols: final_nums = "\n".join(df[cols[0]].astype(str).tolist())
                else: final_nums = file_up.read().decode('utf-8')
            except: pass
        else: final_nums = txt_in
        
        v_sa, v_ph, inv = validate_numbers(final_nums)
        all_v = v_sa + v_ph
    ...
    with col_msg:
        ...
        msg_txt = st.text_area(t("wa_msg_body", lang), height=150)
    ...
    if st.session_state.wa_running:
        ...
        is_ready = status == "Connected" and all_v and msg_txt
    ...
    if st.session_state.wa_running and all_v:
        ...
        if st.session_state.wa_idx < len(all_v):
            ...
            st.write(f"Sending to: {target}...")
            ok, info = st.session_state.wa_service.send_message(target, msg_txt)
    ...
    if st.session_state.wa_logs:
        st.markdown('<div style="background:rgba(0,0,0,0.5); padding:10px; border-radius:10px; max-height:200px; overflow-y:auto; color:#00FF41; font-family:monospace;">', unsafe_allow_html=True)

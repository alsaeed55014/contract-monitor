...
def render_whatsapp_page():
    ...
    if status == "Awaiting Login":
        ...
        # Fix: Using get_qr_hd to match service.py
    ...
    if status == "Connected":
        ...
        with col_input:
            ...
            if all_valid:
                ...
                if invalid: st.warning(f"⚠️ Skipped {len(invalid)} invalid")
        ...
        if st.session_state.wa_running:
            ...
            can_send = len(all_valid) > 0 and message_body
        ...
        if st.session_state.wa_running and all_valid:
            st.progress(st.session_state.wa_sent_count / len(all_valid))
            ...
            if st.session_state.wa_sent_count < len(all_valid):
                ...
                # Double-checking function name in whatsapp_service.py

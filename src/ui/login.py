...
class LoginScreen:
    def __init__(self, root, auth_manager, on_login_success):
        ...
        self.root.geometry("400x520") # تقليل ارتفاع النافذة الكلي
        self.root.config(bg=COLORS["bg_main"])
        
        self.center_window()

        # Main Container (Centered Card) - تقليل الـ padding
        self.card = tk.Frame(self.root, bg=COLORS["bg_secondary"], width=350, padx=15, pady=15)
        self.card.place(relx=0.5, rely=0.5, anchor="center")
        self.card.pack_propagate(False)

        # 1. Avatar Image (Circular 55px) - تصغير إضافي للصورة
        self.setup_avatar()

        # 2. Programmer Name (Micro size)
        tk.Label(self.card, text=PROGRAMMER_NAME_EN.upper(), 
                 font=("Segoe UI", 7, "bold"), 
                 bg=COLORS["bg_secondary"], fg=COLORS["accent"]).pack(pady=(2, 0))

        # 3. Title (Compact)
        tk.Label(self.card, text="أهلاً بك", 
                 font=("Segoe UI", 16, "bold"), 
                 bg=COLORS["bg_secondary"], fg=COLORS["white"]).pack(pady=(2, 0))
        
        tk.Label(self.card, text="نظام مراقبة العقود", 
                 font=("Segoe UI", 8), 
                 bg=COLORS["bg_secondary"], fg=COLORS["text_dim"]).pack(pady=(0, 10))

        # 4. Input Fields (Tight Spacing)
        ...
        self.user_entry.pack(fill=tk.X, pady=3, ipady=4) 
        ...
        self.pass_entry.pack(fill=tk.X, pady=3, ipady=4)
        self.pass_entry.bind('<Return>', self.do_login)

        # 5. Login Button (Compact height)
        self.login_btn = LuxuryButton(self.card, text="دخول", command=self.do_login)
        self.login_btn.config(font=("Segoe UI", 10, "bold"), pady=5) 
        self.login_btn.pack(fill=tk.X, pady=(10, 0))

        # 6. Language Switcher (Small)
        lang_btn = tk.Button(self.card, text="عربي / English", font=("Segoe UI", 8),
                            bg=COLORS["bg_main"], fg=COLORS["white"], relief="flat", bd=1)
        lang_btn.pack(pady=(12, 0))

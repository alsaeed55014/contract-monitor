...
def show_toast(message, typ="success", duration=5, container=None):
    """
    Renders a floating luxury notification (popup).
    - If container is None: Pops up at the TOP of the screen (Fixed).
    - If container is provided: Pops up NEAR the call site (Contextual).
    """
    is_contextual = container is not None
    pos_css = "position: absolute; top: -60px; right: 0; width: 320px;" if is_contextual else "position: fixed; top: 30px; left: 50%; transform: translateX(-50%); width: 90%; max-width: 420px;"
    ...
    wrapper_start = '<div style="position: relative; width: 100%; height: 0;">' if is_contextual else ''
    wrapper_end = '</div>' if is_contextual else ''

    # CSS for the Floating Luxury Popup with Entrance/Exit Animations
    toast_html = f"""
    {wrapper_start}
    <div style="{pos_css} z-index: 9999999; pointer-events: none; direction: rtl;">
        <div style="
            background: {bg};
            border: 1.8px solid {accent};
            border-radius: 12px;
            box-shadow: 0 20px 40px rgba(0,0,0,0.8), 0 0 15px {accent}11;
            display: flex;
            align-items: center;
            padding: 10px 16px;
            gap: 12px;
            animation: luxuryToastPopLocal 4.5s cubic-bezier(0.1, 0.9, 0.2, 1) forwards;
            overflow: hidden;
            min-height: 50px;
        ">
            <style>
                @keyframes luxuryToastPopLocal {{
                    0%   {{ opacity: 0; transform: translateY(20px) scale(0.9); }}
                    8%   {{ opacity: 1; transform: translateY(0) scale(1.02); }}
                    12%  {{ transform: scale(1); }}
                    88%  {{ opacity: 1; transform: translateY(0); }}
                    100% {{ opacity: 0; transform: translateY(-15px) scale(0.98); visibility: hidden; }}
                }}
            </style>
            <div style="width: 24px; height: 24px; flex-shrink: 0;">
                <svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" fill="{accent}22"/>{icon_path}</svg>
            </div>
            <div style="color: #FFF; font-size: 14px; font-weight: 700; flex-grow: 1; text-align: right; font-family: 'Cairo', sans-serif; line-height: 1.2;">
                {message}
            </div>
        </div>
    </div>

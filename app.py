...
try:
    ...
    from src.ui.styles import get_base64_image as load_base64_image
    from src.utils.data_utils import find_row_value, get_flag_emoji
    ...
    from ui.styles import get_base64_image as load_base64_image
    from utils.data_utils import find_row_value, get_flag_emoji
...
get_base64_image = st.cache_data(ttl=3600, show_spinner=False)(load_base64_image)
    ...
    try:
        ...
        if current_worker_count > last_worker:
            ...
            for _, row in new_rows.iterrows():
                name = str(find_row_value(row, ["الاسم", "Name", "Full Name", "الاسم الكامل"]))
                nat = str(find_row_value(row, ["الجنسية", "Nationality"]))
                job = str(find_row_value(row, ["نوع العمل", "الوظيفة", "Job", "Job Type"]))
                loc = str(find_row_value(row, ["المدينة", "Location", "City"]))
                phone = str(find_row_value(row, ["رقم الجوال", "Phone", "Mobile", "الهاتف"]))
                age = str(find_row_value(row, ["العمر", "Age"]))
                religion = str(find_row_value(row, ["الديانة", "Religion"]))
        ...
        if current_cust_count > last_cust:
            ...
            for _, row in new_rows.iterrows():
                # Find company name
                company = str(find_row_value(row, ["اسم الشركه", "المؤسسة", "Company", "الشركة", "شركه"]))
                # Find contact person
                pic = str(find_row_value(row, ["المسئول", "Contact Person", "Person in charge", "المسؤول"], ''))
                # Find phone
                phone = str(find_row_value(row, ["رقم الجوال", "Phone", "Mobile", "الهاتف", "الجوال"], ''))
                # Find gender
                gender_val = str(find_row_value(row, ["الفئة المطلوبة", "الجنس", "Gender", "الفئة"]))
                # Find nationality
                nat = str(find_row_value(row, ["الجنسية المطلوبة", "الجنسية", "Nationality"]))
                # Find location
                loc = str(find_row_value(row, ["موقع العمل", "الموقع", "Location", "المدينة"]))
                # Find salary
                salary = str(find_row_value(row, ["الراتب", "Salary", "Expected salary"]))
                # Find experience
                exp = str(find_row_value(row, ["الخبرة", "Experience"]))
                # Find contract type
                contract = str(find_row_value(row, ["نوع العقد", "Contract Type"]))
        ...
        with av_col2:
            ...
            if uploaded_photo:
                try:
                    ...
                    # Preview
                    prev_buf = io.BytesIO()
                    img_cropped.convert("RGB").save(prev_buf, format="JPEG", quality=85)
                    prev_b64 = base64.b64encode(prev_buf.getvalue()).decode('utf-8')
                    st.markdown(
                        f'<div style="display:flex;justify-content:center;margin:10px 0;">'
                        f'<img src="data:image/jpeg;base64,{prev_b64}" '
                        f'style="width:150px;height:150px;border-radius:50%;object-fit:cover;border:3px solid #D4AF37;" /></div>',
                        unsafe_allow_html=True
                    )
                    st.caption("👆 " + ("معاينة الصورة - سيتم اقتصاصها تلقائياً بشكل دائري" if lang == 'ar' else "Preview — auto center-cropped to circle"))
                    
                    if st.button("💾 " + ("حفظ الصورة" if lang == 'ar' else "Save Photo"), key="save_avatar_btn", type="primary"):
                        buf = io.BytesIO()
                        img_cropped.convert("RGB").save(buf, format="JPEG", quality=85)
                        avatar_bytes = buf.getvalue()
                        avatar_b64 = base64.b64encode(avatar_bytes).decode('utf-8')
                        full_uri = f"data:image/jpeg;base64,{avatar_b64}"

...
def render_bengali_supply_content():
    ...
    with tab3:
        ...
        st.markdown("#### 🔍 البحث الموحد في كل البيانات")
            ...
            if results:
                ...
                for w in sorted(results, key=lambda x: x.get("timestamp", ""), reverse=True):
                    ...
                    df_g.append({
                        ...
                        "هوية العامل": w.get("id", ""),
                        "المورد": s_info.get("name", w.get("supplier")),
                        "جوال المورد": s_info.get("phone", ""),
                        "صاحب العمل": e_info.get("name", w.get("employer")),
                        "جوال صاحب العمل": e_info.get("mobile", ""),
                        "المقهى": e_info.get("cafe", ""),
                        "المدينة": e_info.get("city", ""),
                        "التاريخ": w.get("timestamp", "")
                    })
                st.dataframe(pd.DataFrame(df_g), use_container_width=True, hide_index=True)
                
                st.info("💡 يمكنك طباعة هذه الصفحة بالضغط على Ctrl + P. لتكبير الصور، اضغط على أي صورة أدناه.")
                
                st.markdown("---")
                st.markdown("### 📄 تفاصيل السجلات المترابطة")
                
                # 2. Detailed Cards with Images
                for w in sorted(results, key=lambda x: x.get("timestamp", ""), reverse=True):
                    with st.expander(f"👷 {w.get('name', 'N/A')} - {w.get('id', '')}", expanded=True):
                        ...
                        with c2:
                            ...
                            st.write(f"المدينة: {e_info.get('city', 'N/A')}")
                            st.write(f"الجوال: {e_info.get('mobile', 'N/A')}")
                        with c3:
                            st.markdown("**📦 المورد**")
                            st.write(f"الاسم: {s_info.get('name', 'N/A')}")
                            st.write(f"الجوال: {s_info.get('phone', 'N/A')}")
                        
                        if w.get('file_notes') or w.get('general_notes'):
                            st.markdown("**📝 ملاحظات**")
                            if w.get('file_notes'): st.write(f"*ملفات:* {w['file_notes']}")
                            if w.get('general_notes'): st.write(f"*عامة:* {w['general_notes']}")

                        # Images
                        saved_files = w.get("files", [])
                        if saved_files:
                            st.markdown("#### 📎 المرفقات والصور")
                            cols = st.columns(2)
                            for i, sf in enumerate(saved_files):
                                ...
                                if path and os.path.exists(path):
                                    ...
                                    if ext in ["jpg", "jpeg", "png", "webp"]:
                                        c.image(path, caption=sf.get("name"), use_column_width=True)
                                    else:
                                        with open(path, "rb") as f:
                                            c.download_button(f"تحميل {sf.get('name')}", data=f, file_name=sf.get("name"), key=f"dl_{w_uuid}_{i}")
                        
                        # Delete button (if perms allowed)
                        if can_delete:
                            if st.button("🗑️ حذف السجل بالكامل", key=f"del_global_{w_uuid}", use_container_width=True):
                                if bm.delete_worker(w_uuid):
                                    st.success("تم الحذف بنجاح")
                                    time.sleep(0.5)
                                    st.rerun()
            else:
                st.warning("لم يتم العثور على أي بيانات تطابق هذا البحث")
        else:
            st.info("💡 اكتب اسم العامل أو المورد أو صاحب العمل للبحث عن كافة البيانات المربوطة")
            
            # Show stats for context when no search is active
            col1, col2, col3 = st.columns(3)
            col1.metric("👷 إجمالي العمال", len(workers_all))
            col2.metric("📦 الموردين", len(bm.get_suppliers()))
            col3.metric("🏢 أصحاب العمل", len(bm.get_employers()))
